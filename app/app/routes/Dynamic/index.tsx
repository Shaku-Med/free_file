...
import { useMusicMix } from "~/lib/hooks/useMusicMix";
...
import { ChevronDown, MessageCircle, ListVideo } from "lucide-react";
...
const DynamicPage = ({ is_modal }: DynamicPageProps) => {
  ...
  // YouTube-style Mix: the up-next is an auto-generated radio seeded from the
  // current track — it's just there, no user action (see useMusicMix /
  // get_music_mix RPC). Distinct from the manual play queue.
  const musicMix = useMusicMix(currentId);
  ...
  const handleVideoSelect = useCallback((video: FileType) => {
    // Inside a Mix, every hop (manual click or autoplay) keeps the `list=RD…`
    // station so the queue persists and the link stays shareable.
    navigate(
      musicMix.active ? musicMix.buildHref(video.unique_id) : `/${video.unique_id}`,
    );
  }, [navigate, musicMix])

  const relatedVideos =
    musicMix.active && musicMix.videos.length > 0
      ? musicMix.videos
    ...
    <aside className="min-w-0 lg:col-span-1">
      <div className="space-y-4 lg:sticky lg:top-6">
        ...
        <div className="min-w-0 rounded-lg border border-border/40 bg-card/30 p-2 sm:p-3 lg:border-0 lg:bg-transparent lg:p-0">
          {musicMix.active && musicMix.videos.length > 0 ? (
            <div className="mb-2 flex items-center gap-2 rounded-xl border border-border/50 bg-card/60 px-3 py-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  Autogenerated for you · {musicMix.videos.length} tracks
                </p>
              </div>
            </div>
          ) : null}
