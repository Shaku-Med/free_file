  ...
  async getFeedFiles(feedItems: FeedItem[]): Promise<any[]> {
    try {
      ...
      const { data, error } = await db
        .from('files')
        .select('id, filename, unique_id, up_count, down_count, views, view_count, shares, share_count, file_size, file_type, endpoint, created_at, is_adult, is_public, owner_id, thumbnails, file_title, category, file_description')
        .in('id', fileIds);
