-- ============================================================
-- SEARCH V8: typo-tolerant titles + semantic vectors + v4 lexical
-- ============================================================
-- v8 = v7 + word-level typo matching on TITLES: "got gen" now finds
-- "GUT GENUG". Every query token must prefix-match or sit within edit
-- distance 1 (<=4 chars) / 2 (longer) of some title word (fuzzystrmatch
-- levenshtein). Trigram fuzzy alone can't do this  3-letter typos share
-- almost no trigrams ("got"/"gut") and Supabase blocks tuning the GUC.
--
...
BEGIN
  ...
  -- Tokens of the RAW query (not the number-expanded norm) for the
  -- typo-tolerant title branch. Max 6 tokens bounds the per-row work.
  SELECT array_agg(t) INTO v_tokens FROM (
    SELECT t
    FROM unnest(regexp_split_to_array(lower(btrim(COALESCE(p_query, ''))), '[^a-z0-9]+')) AS t
  ...
  matches AS (
    SELECT
      ...
      CASE WHEN fz.hit THEN 1.0 ELSE 0.0 END                                  AS _fuzzy_title
    FROM files f
    ...
    -- Typo-tolerant title check: every query token prefix-matches or sits
    -- within edit distance 1 (<=4 chars) / 2 of some title word.
    CROSS JOIN LATERAL (
      SELECT (
        ...
        AND NOT EXISTS (
          ...
          WHERE NOT EXISTS (
            SELECT 1
            FROM unnest(regexp_split_to_array(lower(COALESCE(f.file_title, '')), '[^a-z0-9]+')) AS tw(w)
            WHERE tw.w <> ''
              AND (
                tw.w LIKE qt.tok || '%'
                OR (
