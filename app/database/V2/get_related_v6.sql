-- get_related v6 — LANE KEEPING + semantic "same vibe" ranking.
--
-- Problem solved: autoplay (up next = top of related) could jump from a
-- music video to anything. v5 added:
--
--   1. HARD content lane: candidates sharing at least one category with the
--      current video always rank BEFORE everything else. Watching Music →
--      every music video in the catalog surfaces before any non-music one;
--      the lane only "breaks" when there is literally nothing left in it.
--      (Videos with no categories on the current file have no lane — all
--      candidates compete equally, exactly like v4.)
--   2. Vibe score: when both files have embeddings (bge-small via EmbedAPI),
--      cosine similarity adds up to ~20 points — so within the Music lane,
--      Afrobeats stays with Afrobeats, not with metal.
--
-- Everything else (session seed rotation, creator caps, interest/affinity,
-- negative signals, score-bucket shuffle) is unchanged from v4.
--
-- v6 adds playback-depth weighting: an action taken late in a video counts for
-- more than one taken at the thumbnail. See file_engagement_stats_v2.sql.
...
BEGIN
  ...
  -- A lane only exists when the current video actually has categories.
  ...
  candidates AS (
    SELECT
      ...
      -- HARD lane: 0 = same content lane as the current video (or no lane
      -- exists), 1 = different lane. Sorted on FIRST, so music exhausts
      -- before anything else ever shows.
      CASE
      ...
      -- Same vibe: embedding cosine similarity (0 when either is missing).
      CASE
      ...
      -- Soft penalty for creators the user already saw this session.
      ...
      -- v6: how deep into playback people were when they acted on this
      -- file. Rewards content that earns its engagement over content that
      -- collects it from the thumbnail.
  ...
  ranked AS (
    SELECT c.*,
      ROW_NUMBER() OVER (
        ORDER BY
          -- Lane first: same-category content ALWAYS beats off-lane content.
          c._lane ASC,
