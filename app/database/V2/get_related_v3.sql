...
-- Signature is identical to v2, no app changes needed.
-- ============================================================
...
BEGIN
  ...
  -- NEW: explicit subscription graph
  ...
  candidates AS (
    SELECT
      ...
      -- Content relevance to currently-playing file
      (
      ...
      -- NEW: subscribed-creator boost
      CASE WHEN sc.creator_id IS NOT NULL THEN 35.0 ELSE 0.0 END AS _sub_boost,

      -- NEW: recency boost for fresh uploads from subscribed creators
      -- Linear decay over 30 days, peaks at +25 for same-day uploads.
      CASE
  ...
  capped AS (
    SELECT s.*,
      ROW_NUMBER() OVER (
        ...
        ORDER BY s._score DESC, s.created_at DESC, s.id
      ) AS _per_creator_rn
    FROM scored s
  ),
  ...
  ranked AS (
    SELECT c.*,
      ROW_NUMBER() OVER (
        ORDER BY
          -- Tier 1: top picks per creator (top 4 from source-video creator,
          -- top 2 from everyone else). Tier 2: everything else.
